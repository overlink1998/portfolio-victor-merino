...
class MainWindow(QMainWindow):
    ...
    def cargar_configuracion_negocios(self):
        ...
        try:
            ...
            print(f"[Depuración] {len(self.negocios)} negocios cargados.")
            return self.negocios
        except FileNotFoundError:
            self.negocios = []
            print("[Error] No se encontró el archivo 'negocios_config.json'. Se inicializa una lista vacía de negocios.")
            return self.negocios
        except json.JSONDecodeError:
            QMessageBox.critical(self, "Error", "El archivo 'negocios_config.json' está corrupto.")
            self.negocios = []
            print("[Error] El archivo 'negocios_config.json' está corrupto.")
            return self.negocios
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error al cargar los negocios: {e}")
            self.negocios = []
            print(f"[Error] Error al cargar los negocios: {e}")
            return self.negocios
    ...
    def abrir_negocio(self, item_negocio):
        ...
        if item_negocio:
            nombre_negocio = item_negocio.text()
            negocios = self.cargar_configuracion_negocios()
            self.current_negocio = nombre_negocio 
    ...
    def agregar_a_cotizacion(self, tree_view):
        ...
        if indexes:
            ...
            # Obtener el negocio actual de la pestaña
            pestaña = self.notebook.currentWidget()
            negocio_nombre = getattr(pestaña, 'negocio', None)
    
            if not negocio_nombre:
                # Intentar inferir 'negocio' desde el file_path si no está establecido
                file_path = getattr(pestaña, 'file_path', None)
                if file_path:
                    negocio_nombre = os.path.basename(os.path.dirname(file_path))
    
            if not negocio_nombre:
                # Si 'negocio' aún no está determinado, solicitar al usuario que lo seleccione
                negocios_nombres = [negocio['nombre'] for negocio in self.negocios]
                negocio_nombre, ok = QInputDialog.getItem(self, "Seleccionar Negocio", "Seleccione el negocio para la cotización:", negocios_nombres, 0, False)
                if not (ok and negocio_nombre):
                    QMessageBox.warning(self, "Error", "Debe seleccionar un negocio para agregar a la cotización.")
                    print("Error: No se pudo determinar o seleccionar el negocio.")
                    return
    
            print(f"Negocio actual: {negocio_nombre}")
    
            # Mostrar el diálogo para seleccionar la cotización
    ...
    def abrir_pestaña_cotizacion_by_id(self, cotizacion_id):
        ...
        # Supongamos que 'cotizaciones' es una lista accesible dentro de la clase
        cotizacion = next((c for c in cotizaciones if c['id'] == cotizacion_id), None)
        if not cotizacion:
        ...
        if not negocio_nombre:
            QMessageBox.warning(self, "Error", f"No hay un negocio asociado a la cotización {cotizacion_id}.")
            return
    
        print(f"Negocio asociado a la cotización {cotizacion_id}: {negocio_nombre}")  # Depuración
        ...
        # Conectar el botón con la función de búsqueda, pasando la cotización actual y el input
        ...
        # Puedes conectar este botón de manera similar si necesitas filtrar responsables por negocio
    
        centro_label = QLabel("Centro de costo:")
        centro_input = QLineEdit(cotizacion.get('centro_costo', ''))
        centro_buscar_btn = QPushButton("🔍")
        # Puedes conectar este botón de manera similar si necesitas filtrar centros de costo por negocio
        ...
        # Tabla de productos
        model_productos = QStandardItemModel()
        model_productos.setHorizontalHeaderLabels([
        ...
        for producto in cotizacion['productos']:
            ...
            for col, item in enumerate(items):
                if col in [0, 1, 6, 7, 8]:  # No. Producto, Descripción, Impuestos, Valor Total
                    ...
                    item.setEditable(True)
            model_productos.appendRow(items)
    
        tree_productos = QTreeView()
        tree_productos.setModel(model_productos)
        ...
        model_productos.dataChanged.connect(lambda topLeft, bottomRight, roles: self.on_product_data_changed(
            model_productos, topLeft, bottomRight, cotizacion))
        ...
        layout.addWidget(btn_agregar)
        # **Corrección Aquí: Pasar 'nueva_pestaña' en lugar de 'cotizacion_dialog'**
        btn_agregar.clicked.connect(lambda: self.agregar_cliente_desde_dialog(nueva_pestaña, negocio_nombre))
    ...
    def buscar_o_agregar_cliente_para_cotizacion(self, cotizacion, cliente_input):
        ...
        dialog.clientes_model = model  # Guardar referencia al modelo
        dialog.negocio_nombre = negocio_nombre  # Guardar el nombre del negocio
    
        # Conectar el botón de agregar cliente
        btn_agregar.clicked.connect(lambda: self.agregar_cliente_desde_dialog(dialog, negocio_nombre, cotizacion_tab=self.notebook.currentWidget(), cliente_dialog=dialog))
    ...
    def cargar_clientes_del_negocio(self, negocio_nombre):
        """
        Carga los clientes asociados a un negocio específico.
        """
        if not negocio_nombre or not isinstance(negocio_nombre, str):
            QMessageBox.warning(self, "Error", "Nombre de negocio inválido.")
            return None
    
        # Obtener el directorio base donde se encuentra el script
        base_dir = os.path.dirname(os.path.abspath(__file__))
    
        # Construir la ruta al archivo de clientes
        file_path = os.path.join(base_dir, 'negocios', negocio_nombre, f'clientes_{negocio_nombre}.xlsx')
    
        print(f"[Depuración] Ruta de clientes para el negocio '{negocio_nombre}': '{file_path}'")
    
        # Verificar si el archivo existe
        if os.path.exists(file_path):
            print(f"[Depuración] Archivo de clientes encontrado en '{file_path}'. Intentando cargarlo...")
            try:
                # Cargar el archivo Excel con pandas
                df = pd.read_excel(file_path, dtype={'Identificacion': str})
                print(f"[Depuración] {len(df)} clientes cargados para el negocio '{negocio_nombre}'.")
                return df
            except Exception as e:
                QMessageBox.critical(
                    self, 
                    "Error", 
                    f"No se pudo cargar el archivo de clientes para el negocio '{negocio_nombre}'.\nError: {e}"
                )
                print(f"[Error] Al cargar el archivo de clientes: {e}")
                return None
        else:
            print(f"[Depuración] Archivo de clientes no encontrado en '{file_path}'. Creando un nuevo archivo.")
            # Crear un nuevo DataFrame si el archivo no existe
            df = pd.DataFrame(columns=['Identificacion', 'Nombre1', 'Nombre2', 'Apellido1', 'Apellido2'])
            return df
    ...
    def seleccionar_cliente_en_dialog(self, tree_view, cotizacion, cliente_input, dialog):
        """
        Asigna el cliente seleccionado al input de cliente y cierra el diálogo.
        """
        indexes = tree_view.selectedIndexes()
        if indexes:
            row = indexes[0].row()
            ...
            identificacion = model.item(row, 0).text().strip()
            nombre1 = model.item(row, 1).text().strip()
            nombre2 = model.item(row, 2).text().strip()
            apellido1 = model.item(row, 3).text().strip()
            apellido2 = model.item(row, 4).text().strip()
            ...
            # cotizacion['id_cliente'] = identificacion
    ...
    def agregar_cliente_desde_dialog(self, dialog, negocio_nombre, cotizacion_tab, cliente_dialog):
        """
        Abre un diálogo para agregar un nuevo cliente y lo guarda en la base de datos del negocio correspondiente.
        """
        print(f"Agregar cliente para negocio: {negocio_nombre}")  # Depuración
    
        if not negocio_nombre:
            QMessageBox.warning(self, "Error", "No hay un negocio asociado a esta cotización.")
            return
    
        # Crear un nuevo diálogo para agregar cliente
        agregar_dialog = QDialog(self)
        agregar_dialog.setWindowTitle("Agregar Nuevo Cliente")
        agregar_dialog.resize(400, 300)
        layout = QVBoxLayout(agregar_dialog)
    
        # Campos de entrada para el cliente
        identificacion_label = QLabel("Identificación:")
        identificacion_input = QLineEdit()
        nombre1_label = QLabel("Nombre 1:")
        nombre1_input = QLineEdit()
        nombre2_label = QLabel("Nombre 2:")
        nombre2_input = QLineEdit()
        apellido1_label = QLabel("Apellido 1:")
        apellido1_input = QLineEdit()
        apellido2_label = QLabel("Apellido 2:")
        apellido2_input = QLineEdit()
        ...
        layout.addWidget(identificacion_label)
        layout.addWidget(identificacion_input)
        layout.addWidget(nombre1_label)
        layout.addWidget(nombre1_input)
        layout.addWidget(nombre2_label)
        layout.addWidget(nombre2_input)
        layout.addWidget(apellido1_label)
        layout.addWidget(apellido1_input)
        layout.addWidget(apellido2_label)
        layout.addWidget(apellido2_input)
        layout.addLayout(btn_layout)
    
        # Conectar botones
        btn_guardar.clicked.connect(lambda: self.guardar_nuevo_cliente(
        negocio_nombre,
        identificacion_input.text(),
        nombre1_input.text(),
        nombre2_input.text(),
        apellido1_input.text(),
        apellido2_input.text(),
        agregar_dialog,
        cotizacion_tab,
        cliente_dialog
        ))
        btn_cancelar.clicked.connect(agregar_dialog.reject)
    ...
    def guardar_nuevo_cliente(self, negocio_nombre, identificacion, nombre1, nombre2, apellido1, apellido2, agregar_dialog, cotizacion_tab, cliente_dialog):
        """
        Guarda el nuevo cliente en el archivo de clientes del negocio correspondiente y actualiza el diálogo principal.
        """
        # Validar campos obligatorios
        if not identificacion or not nombre1 or not apellido1:
            QMessageBox.warning(self, "Advertencia", "Los campos de identificación, Nombre 1 y Apellido 1 son obligatorios.")
            return
    
        # Validar que 'negocio_nombre' es válido
        if not negocio_nombre or not isinstance(negocio_nombre, str):
            QMessageBox.warning(self, "Error", "Nombre de negocio inválido.")
            print(f"[Error] 'negocio_nombre' es inválido: {negocio_nombre}")
            return
    
        # Crear un DataFrame con el nuevo cliente
        ...
        # Cargar los clientes actuales del negocio
        clientes_df = self.cargar_clientes_del_negocio(negocio_nombre)
        if clientes_df is not None:
            # Verificar si la identificación ya existe
            if clientes_df['Identificacion'].str.lower().eq(identificacion.lower()).any():
                QMessageBox.warning(self, "Error", f"Ya existe un cliente con la identificación '{identificacion}'.")
                print(f"[Advertencia] Cliente con identificación '{identificacion}' ya existe.")
                return
            ...
            # Construir la ruta del archivo de clientes
            base_dir = os.path.dirname(os.path.abspath(__file__))
            file_path = os.path.join(base_dir, 'negocios', negocio_nombre, f'clientes_{negocio_nombre}.xlsx')
    
            print(f"[Depuración] Ruta de archivo de clientes: {file_path}")
    
            # Asegurarse de que el directorio existe
            try:
                # Guardar el DataFrame actualizado en el archivo Excel
                clientes_actualizados.to_excel(file_path, index=False)
                QMessageBox.information(self, "Éxito", "Nuevo cliente agregado correctamente.")
                print(f"[Éxito] Cliente agregado y guardado en '{file_path}'.")
                agregar_dialog.accept()
        
                # Actualizar la lista de clientes en el diálogo existente
                self.actualizar_lista_clientes_en_dialog(cliente_dialog, negocio_nombre)
                print(f"[Depuración] Lista de clientes actualizada en el diálogo.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"No se pudo guardar el nuevo cliente.\nError: {e}")
                print(f"[Error] Al guardar el cliente en '{file_path}': {e}")
            ...
            try:
                ...
                # Recargar los clientes en el diálogo principal
                cotizacion = cotizacion_tab.cotizacion
                cliente_input = cotizacion_tab.cliente_input
            
                if cotizacion:
                    # Actualizar la lista de clientes en la cotización
                    print(f"[Depuración] Lista de clientes actualizada para la cotización ID {cotizacion['id']}.")
                else:
                    print("[Error] No se pudo encontrar la cotización asociada a la pestaña actual.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"No se pudo guardar el nuevo cliente.\nError: {e}")
                print(f"[Error] Al guardar el cliente en '{file_path}': {e}")
        else:
            QMessageBox.warning(self, "Error", f"No se pudieron cargar los clientes para el negocio '{negocio_nombre}'.")
            print(f"[Error] No se pudieron cargar los clientes para el negocio '{negocio_nombre}'.")
            
            
    def actualizar_lista_clientes_en_dialog(self, dialog, negocio_nombre):
        # Cargar los clientes actualizados
        clientes_df = self.cargar_clientes_del_negocio(negocio_nombre)
        if clientes_df is not None:
            # Actualizar el modelo en el diálogo
            model = dialog.clientes_model
            model.clear()
            model.setHorizontalHeaderLabels(clientes_df.columns)
            ...
            for index, row in clientes_df.iterrows():
                ...
                for field in row:
                    ...
                    items.append(item)
                model.appendRow(items)
            print("Lista de clientes actualizada en el diálogo.")
        else:
            print(f"No se pudo cargar la lista de clientes para el negocio '{negocio_nombre}'.")

    
    def find_cotizacion_by_cliente_input(self, dialog_principal):
        """
        Encuentra la cotización asociada al cliente_input en el diálogo principal.
        """
        # Iterar sobre las pestañas para encontrar la cotización correspondiente
        current_widget = self.notebook.currentWidget()
        if hasattr(current_widget, 'cotizacion'):
            return current_widget.cotizacion
        return None
